...
namespace eProcessFactura
{
    public class EInvoice
    {
        ...
        public EInvoice(MainSettings MSI, Validation VDI)
        {
            MS = MSI;
            VD = VDI;
        ...
        public UBLExtensionType[] getUBLExtensions()
        {
            ...
            //TotVtaGrab - TotVtaGrat
            if (Interface["TotVtaGrab"] != null)
            {
                grav = Interface["TotVtaGrab"].ToString().Length > 0 ? Convert.ToDecimal(Interface["TotVtaGrab"].ToString()) : 0.0m;
            }
            if (Interface["TotVtaInaf"] != null)
            {
                inafec = Interface["TotVtaInaf"].ToString().Length > 0 ? Convert.ToDecimal(Interface["TotVtaInaf"].ToString()) : 0.0m;
            }
            if(Interface["TotVtaExon"] != null)
            {
                exoner = Interface["TotVtaExon"].ToString().Length > 0 ? Convert.ToDecimal(Interface["TotVtaExon"].ToString()) : 0.0m;
            }
